...
async def read_contacts(skip: int = 0,
    ...
    contacts = await repositore_contact.get_contacts(skip, limit, current_user, db)
    return contacts
...
async def read_contact(contact_id: int,
                       db: Session = Depends(get_database),
                       current_user: User = Depends(get_current_user)):
    contact = await repositore_contact.get_contact(contact_id, current_user, db)
    if contact is None:
...
async def create_contact(body: ContactModel,
                         db: Session = Depends(get_database),
                         current_user: User = Depends(get_current_user)):
    return await repositore_contact.create_contact(body, current_user, db)
...
async def update_contact(body: ContactModel,
                         ...
                         db: Session = Depends(get_database),
                         current_user: User = Depends(get_current_user)):
    contact = await repositore_contact.update_contact(contact_id, current_user, body, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
